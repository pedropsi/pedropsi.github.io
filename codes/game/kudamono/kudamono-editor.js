...
// Keys(FruitIcons).map(k=>FruitIcons[k].transform=function(x,y){return SVGWidthRescaler(40)(x,y,ViewboxCoordinates(FruitIcons[k].viewBox))});

// Icons={...Icons,...FruitIcons};

// TransformIcon=function(Opts,Transformer){
// 	var path=Opts.path;
// 	var viewBox=Opts.viewBox;
// 	var T=function(x,y){return Transformer(x,y,ViewboxCoordinates(viewBox))};
// 		path=SVGPathTransform(path,T,viewBox);
// 		viewBox=ViewboxCoordinates(viewBox);
// 		viewBox=T(viewBox[0],viewBox[1]).join(" ")+T(viewBox[2],viewBox[3]).join(" ");
// 	return {...Opts,path:path,viewBox:viewBox}
// }
...
RescalePath=function(Opts){
	...
	var Rescale=function(x,y){return RescaleWidthXYer(scale)(x,y,viewBox)};
...
DrawFruits=function(){
	var types=Keys(STATE.fruits);
	types.map(function(type){
		var coordinates=STATE.fruits[type];
