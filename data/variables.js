...
GAME_EXTRAS_TEXT			:	()=>	`${v.TOUCHSCREEN_TEXT()}${v.NAVIGATION_TEXT()}${v.FULLSCREEN_TEXT()}${v.FEEDBACK_TEXT()}${v.COMMUNITY()}`,
GAME_EXTRAS_HINTS_TEXT		:	()=>	`${v.TOUCHSCREEN_TEXT()}${v.NAVIGATION_TEXT()}${v.FULLSCREEN_TEXT()}${v.HINTS_TEXT()}${v.FEEDBACK_TEXT()}${v.COMMUNITY()}`,
NAVIGATION_TEXT				:	()=>	`<h3>Level navigation</h3><p>Press ${v.LEVEL_KEY()} or <kbd>Level X/Y</kbd> or <kbd>Level selector</kbd> in the game bar to navigate freely ${v.NAVOPTION()}.</p><p>Type the level number directly with ${v.NUM_KEYS()}, press <kbd>Tab</kbd>/<kbd>Shift</kbd>+<kbd>Tab</kbd> or ${KB("left")}/${KB("right")} to <em>cycle through levels</em>, <kbd>Enter</kbd> or <kbd>Click</kbd> to <em>confirm</em>, <kbd>Esc</kbd> or ${v.LEVEL_KEY()} to <em>cancel</em>.</p>`,
...
TOUCHSCREEN_TEXT			:	()=>	`<h3>Controls - touchscreen</h3><p>${KB("tap")} to simulate <em>pressing <kbd>X</kbd></em> and <kbd>swipe</kbd> to simulate an <em><kbd>arrow</kbd> keypress</em>.`,
UNDO_TOUCH_TEXT				:	()=>	`<em>undo</em>, `,
UNDO_RESTART_TEXT			:	()=>	`<p>${v.UNDO_TEXT()} ${v.RESTART_TEXT()}</p>`,
RESTART_TEXT				:	()=>	`Press ${v.RESTART_KEY()} or <kbd>${ObtainSymbol("restart")}</kbd> to restart the level.`,
...
UP_KEY						:	()=>	`<kbd>${ObtainSymbol("up")}</kbd> or <kbd>W</kbd>`,
RIGHT_KEY					:	()=>	`${KB("right")} or <kbd>D</kbd>`,
DOWN_KEY					:	()=>	`<kbd>${ObtainSymbol("down")}</kbd> or <kbd>S</kbd>`,
ACTION_KEY					:	()=>	`<kbd>X</kbd>, <kbd>Spacebar</kbd> or ${KB("enter")}`,
UNDO_KEY					:	()=>	`<kbd>Z</kbd>`,
